...
const api = axios.create({
  ...
  // 65s = the backend's Vercel maxDuration (60s) plus a buffer: slow-but-
  // working hunts/generations still complete, while a request that will never
  // get a response fails with a timely, attributable error instead of hanging.
  timeout: 65_000,
})
